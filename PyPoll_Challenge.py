...
# Create a list and create a dictionary where the county is key and votes are turnout.
county_options = []
county_votes = {}
...
with open(file_to_load) as election_data:
    ...
    for row in file_reader:
        ...
        total_votes += 1 # increment by 1
        # Get the county name from each row.
        county_name = row[1]
        # If the county does not match any existing county, add the
        # the county to list. Use .append()
        if county_name not in county_options:
            # Add the county name to the county's list.
            county_options.append(county_name)
            # Track the county voter count.
            county_votes[county_name] = 0
        # Add vote to county's count. this is incremented by 1
...
with open(file_to_save, "w") as txt_file:
    ...
    for county in county_votes:
        ...
        #  Save county's results to text file.
